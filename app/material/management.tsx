...
export default function MaterialManagement() {
  ...
  const scope = "DEMO";
  const schemaRef = useRef<{ hasScope: boolean; hasTotals: boolean } | null>(
    null
  );
  ...
  async function detectSchema() {
    ...
    try {
      ...
      const hasTotals =
        names.includes("total_in") && names.includes("total_out");
  ...
  const onAdd = async () => {
    ...
    if (await isDuplicateName(n)) {
      setError("Nama material sudah ada.");
      return;
    }
  ...
  const onEditSave = async () => {
    ...
    if (await isDuplicateName(n, showEdit.id)) {
      setError("Nama material sudah ada.");
      return;
    }
    try {
      const { hasScope, hasTotals } = await detectSchema();
      if (hasScope || hasTotals) {
        await run(`UPDATE materials SET name=?, unit=? WHERE id=?`, [
          n,
          u,
          showEdit.id,
        ]);
        ...
        await run(
          `UPDATE materials SET name=?, unit=?, updated_at=? WHERE id=?`,
          [n, u, new Date().toISOString(), showEdit.id]
        );
      }
      setShowEdit(null);
      setName("");
  ...
  const onDelete = async (row: Row) => {
    ...
    try {
      const hasMatLedger =
        (
          await all(
            `SELECT name FROM sqlite_master WHERE type='table' AND name='material_ledger'`
          )
        ).length > 0;
      const hasStockLedger =
        (
          await all(
      ...
      if (hasMatLedger) {
        const c = await get<{ c: number }>(
          `SELECT COUNT(*) c FROM material_ledger WHERE material_id=?`,
          [row.id]
        );
        count = Number(c?.c || 0);
      } else if (hasStockLedger) {
        const c = await get<{ c: number }>(
          `SELECT COUNT(*) c FROM stock_ledger WHERE material_id=?`,
          [row.id]
        );
        count = Number(c?.c || 0);
      }
      const doDelete = async (cascade: boolean) => {
        if (cascade) {
          if (hasMatLedger)
            await run(`DELETE FROM material_ledger WHERE material_id=?`, [
              row.id,
            ]);
          if (hasStockLedger)
            await run(`DELETE FROM stock_ledger WHERE material_id=?`, [row.id]);
        }
  ...
  return (
    <View style={{ flex: 1, padding: 16, minWidth: 0, minHeight: 0 }}>
            ...
            >
              <View>
                <Text style={{ fontWeight: "600" }}>
                  {item.name}{" "}
                  <Text style={{ color: "#64748b" }}>({item.unit || "-"})</Text>
                </Text>
                <Text style={{ color: "#475569", fontSize: 12 }}>
                  Masuk: {item.total_in} · Keluar: {item.total_out} · Saldo:{" "}
                  {item.balance}
                </Text>
              </View>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setShowEdit(null)}
      >
        <View
          style={{
            flex: 1,
